...
tensorboard = TensorBoard(log_dir='Norm_Data/logs/{}'.format(time()))
...
def train(model,
    ...
    if not validate:
        for ep in range(epochs):
            ...
            model.fit_generator(train_gen, steps_per_epoch, epochs=1, use_multiprocessing=True, callbacks=[tensorboard])
            if not checkpoints_path is None:
